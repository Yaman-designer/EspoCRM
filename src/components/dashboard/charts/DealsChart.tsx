...
const stages = [
  { name: 'مؤهل', value: 84, color: '#1A90FF' },
  { name: 'اقتراح', value: 62, color: '#7BBFFF' },
  { name: 'تفاوض', value: 48, color: '#12B76A' },
  { name: 'مغلق', value: 54, color: '#F79009' },
  { name: 'خسارة',  value: 36, color: '#F04438' },
]
  ...
  return (
    <div className="rounded-xl border border-border bg-popover px-3 py-2 shadow-design-md text-xs">
      ...
      <p className="mt-0.5 text-muted-foreground">
        {item.value} صفقة &middot; {Math.round((item.value / total) * 100)}%
      </p>
    </div>
  )
}
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="relative flex justify-center">
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">صفقة</p>
        </div>
      </div>
